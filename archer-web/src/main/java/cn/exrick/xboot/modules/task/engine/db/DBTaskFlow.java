...
public class DBTaskFlow extends TaskFlowAdapter {
	...
	private synchronized void distribteNodes(List<DBTaskNode> nodes) {
		...
		for (DBTaskNode node : nodes) {
            ...
            if(executingTaskCount <= 0 && !instance.getFinished().equals(TaskConstant.INSTANCE_FINISH)){
	...
	private boolean executeNode(DBTaskNode node) {
		//如果没有流程记录则新建一条
        TaskProcess taskProcess = processService.findByTaskIdAndExecuteNode(instance.getId(),
                node.getSemphone().toString());

        //todo 移到后面
        if(taskProcess == null){
            taskProcess = new TaskProcess();
            taskProcess.setTaskId(instance.getId());
            taskProcess.setExecuteNode(node.getSemphone().toString());
            //添加前驱节点
